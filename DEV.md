...
- MMOSの画面を開きっぱなしにするとメモリ使用量が継続的に増加する（リーク疑い、2026-02-01報告）
- ~~MODアプリのインストール状況が再起動/ワールド再読み込みで失われる（installedModAppsの永続化が未実装、2026-02-01報告）~~ → **解決済み（2026-02-01）**: `system/installed_mod_apps.json`への永続化機能を実装
...
\
## 変更(2026-01-27)
- **SettingsScreen UIアーキテクチャの刷新**
  - 問題: パネル（Appearance, Battery等）が半透明で描画され、背面のメインリストと重なって表示されるバグがあった。また、各パネルでヘッダー描画ロジックが重複・散逸していた。
  - 修正内容:
    - **描画ロジックの分離**: draw()メソッドをリファクタリングし、サブパネル表示中はメインリストの描画をスキップするように変更。これにより透過バグを解消。
    - **ヘッダーの統一**: すべてのサブパネルで drawPanelHeader() を使用するように統一。手動でのテキスト描画を削除し、UIの一貫性を向上。
    - **重複描画の削除**: About System, Sound & Vibration, Storage パネルにおいて、Panel クラスと手動描画で二重に描画されていたコンポーネントの描画処理を削除。パフォーマンスを改善。\
\
## 変更(2026-01-27) 追記
- **SettingsScreen UI微修正**
  - Appearanceパネルで消えていたAccent Colorプリセットの手動描画処理を復元。
  - Notificationsパネルで説明テキストがスイッチと重なる問題を修正。Labelコンポーネントを使用して自動レイアウトに対応させた。\
\
## 変更(2026-01-27) 追記2
- **SettingsScreen レイアウトのコンパクト化**
  - 設定一覧の各アイテムの高さを 70 -> 52 に縮小。これにより、全アイテム（About System 含む）が画面内に収まるようになった。
  - 各サブパネルの高さ（ph）を 480 -> 440 に縮小。画面下部のシステムジェスチャー領域との干渉を防いだ。
  - About System パネル内の項目間隔を調整し、縮小されたパネル内にすべての情報が収まるように修正。
  - システム情報（OS名、バージョン等）の表示位置を、設定リストの直後に動的に配置されるよう改善。\
