...
public class LayoutManager {
    ...
    public boolean saveLayout(List<HomePage> pages) {
        try {
            List<PageLayoutData> layoutData = new ArrayList<>();
            ...
            if (success) {
                System.out.println("LayoutManager: レイアウトを保存しました (" + 
                                 pages.size() + "ページ, " + 
                                 layoutData.stream().mapToInt(p -> p.shortcuts.size()).sum() + "ショートカット)");
            }
    ...
    public List<HomePage> loadLayout() {
        try {
            ...
            List<PageLayoutData> layoutData = gson.fromJson(jsonData, listType);
